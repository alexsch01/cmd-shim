...
const writeShim_ = (from, to, prog, args, variables) => {
  ...
  if (shLongProg) {
    pwsh = pwsh
         ...
         + '}\n'
         + '\n'
         + `$PROGRAM_EXE_ARGS='${args}'\n`
         + `$PROGRAM_FILE=${shTarget}\n`
         + '\n'
         + 'if ($MyInvocation.ExpectingInput) { # takes pipeline input\n'
         + '  $input | & "$PROGRAM_EXE" $PROGRAM_EXE_ARGS "$PROGRAM_FILE" $args\n'
         + '} elseif (-not $MyInvocation.Line) { # used "-File" argument\n'
         + '  & "$PROGRAM_EXE" $PROGRAM_EXE_ARGS "$PROGRAM_FILE" $args\n'
         + '} else { # used "-Command" argument\n'
         + '  if ($MyInvocation.Statement) {\n'
         + '    $ORIGINAL_COMMAND = $MyInvocation.Statement\n'
         + '  } else {\n'
         ...
         + '  $PROGRAM_FILE_ARGS = $NO_REDIRECTS_COMMAND.Substring($MyInvocation.InvocationName.Length).Trim()\n'
         + '\n'
         + '  Invoke-Expression "& `"$PROGRAM_EXE`" $PROGRAM_EXE_ARGS `"$PROGRAM_FILE`" $PROGRAM_FILE_ARGS"\n'
         + '}\n'
         + '\n'
         + 'exit $LASTEXITCODE\n'
  } else {
